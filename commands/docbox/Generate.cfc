...
component {
	...
	function run(
		...
		if (
			structKeyExists( arguments, "mappings" ) && isStruct( arguments.mappings ) && structCount(
				arguments.mappings
			)
		) {
			for ( var key in arguments.mappings ) {
				arrayAppend(
					docboxSourceMaps,
					{
						"dir"     : variables.fileSystemUtil.resolvePath( arguments.mappings[ key ] ),
						"mapping" : key
					}
				);
			}
			...
			arrayAppend(
				docboxSourceMaps,
				{
					"dir"     : variables.fileSystemUtil.resolvePath( arguments.source ),
		...
		for ( var tuple in docboxSourceMaps ) {
			if ( !directoryExists( tuple.dir ) ) {
				...
				print.yellowLine( "Source: #tuple.dir#" ).yellowLine( "Mapping: #tuple.mapping#" );
			}
		}
